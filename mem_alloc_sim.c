...
static int look_for_block(int size)
{
    ...
    while(begin_address < MEMORY_SIZE){
        ...
        if(end_address - begin_address > REALSIZE(2 * sizeof(mem_block))){
            valid_address=1;
        }
        else{
            int i=0;
            for(i=0; i< REALSIZE(2 * sizeof(mem_block))-(end_address - begin_address); i++){
                if(memory[end_address+i]==1){
                    break;
                }
            }
            if(end_address - begin_address +i == REALSIZE(2 * sizeof(mem_block))){
                valid_address=1;
            }   
